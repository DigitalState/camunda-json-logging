# Camunda BPM Tomcat 7.10.0 JSON Logging: Docker Image Pattern

A image which pulls Camunda 7.10.0-Tomcat and extends the image with proper configuration for Logging in a JSON format.
...
Logback and slf4j-api dependencies were based on the [parent pom.xml](https://github.com/camunda/camunda-bpm-platform/blob/7.10.0/parent/pom.xml#L24-L25) from the camunda-bpm-platform repository for 7.10.0 tag.
...
1. Pull the Camunda BPM Platform 7.10.0 Tomcat image from DockerHub
...
`docker pull digitalstate/camunda-bpm-platform:tomcat-7.10.0-json-logging` 

Within another dockerfile:

```dockerfile
FROM digitalstate/camunda-bpm-platform:tomcat-7.10.0-json-logging
...
```
...
`docker run --name camunda -p 8080:8080 digitalstate/camunda-bpm-platform:tomcat-7.10.0-json-logging`

or use a dockerfile similar to the ./docker-compose.yml, but swap `build:` with `image: digitalstate/camunda-bpm-platform:tomcat-7.10.0-json-logging`
...
`docker run --name camunda -p 8080:8080 -e PRETTY_JSON_LOG=true digitalstate/camunda-bpm-platform:tomcat-7.10.0-json-logging`


# Building a new image

`docker build -t digitalstate/camunda-bpm-platform:tomcat-7.10.0-json-logging .`
